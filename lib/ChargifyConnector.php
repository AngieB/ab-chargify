...
class ChargifyConnector
{
  ...
  public function retrievePaymentProfileXMLByID($id)
  {
    return $this->sendRequest('/payment_profiles/' . $id . '.xml');
  }
  ...
  */
  ...
  /*** ChargifyPaymentProfile AB 
  public function getPaymentProfileBySubscription($id)
  {
	  $xml = $this->retrievePaymentProfileXMLByID($id);
	  $payment_xml_node = new SimpleXMLElement($xml);
	  $payment_profile = new ChargifyPaymentProfile($payment_xml_node);
    echo "<pre>";
    _log("is var dump happening?");
    var_dump($payment_profile);
    echo "</pre>";
    _log($payment_profile);
}*/
  /**
   * @return ChargifyCustomer
   */
   ...
   public function getSubscriptionsByCustomerID($id)
  {
   ...
   $pay_id = $subscriptions->subscription->credit_card[0]->id;
    echo $pay_id;
    _log($pay_id);
    _log("yes, it is");
  ...
  /*** NEW SUBSCRIPTION FOR USER WITH PAYMENT PROFILE ****/
  public function newSubscription($id) {
    return $this->sendRequest('/subscriptions/' . $id . '.xml', $post_xml, $pay_id );
  }
